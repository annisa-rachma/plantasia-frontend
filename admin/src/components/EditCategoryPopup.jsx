...
export default function EditCategoryPopup({ visible, onClose, id }) {
  ...
  const category = useSelector((state) => {
    return state.categoryReducer.categoryById;
  });
  ...
  const handleSubmit = async (event) => {
    try {
      ...
      await dispatch(handleEditCategory(input, id));
      setInput({
        name: "",
      });
      toast.success("Successfully edit selected category", {
        position: "top-right",
        autoClose: 2000,
